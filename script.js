...
const 头像容器 = document.getElementById('头像容器');
const 个人信息容器 = document.getElementById('个人信息容器');
    ...
    if (configIssue) {
        try {
            ...
            if (config.avatar && config.name && config.bio) {
                ...
                avatarImg.alt = '头像';
                avatarImg.className = '头像';
                头像容器.appendChild(avatarImg);
                ...
                个人信息容器.appendChild(nameElement);
                个人信息容器.appendChild(bioElement);
            } else {
...
function renderSummaries(issues) {
    ...
    issues.forEach((issue, index) => {
        ...
        summaryElement.classList.add('博客卡片');
        // 使用 Flexbox 布局
        ...
        // 将文章概要容器添加到博客文章概要容器
        博客摘要容器.appendChild(summaryElement);
    });
}
